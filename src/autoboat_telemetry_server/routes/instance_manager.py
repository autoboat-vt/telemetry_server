...
class InstanceManagerEndpoint:
    ...
    def _register_routes(self) -> str:
        ...
        def set_instance_archive(instance_id: int, archive_status: int) -> tuple[Response, int]:
            ...
            ----------
            instance_id
                The ID of the telemetry instance to set the archive status for.
            archive_status
                The archive status to assign to the telemetry instance (0 for False, 1 for True).

            Returns
            -------
            tuple[Response, int]
                A tuple containing a JSON response confirming the archive status has been set and a status code of 200.
            """
            ...
            try:
                ...
                telemetry_instance.archive = archive_status == 1
                db.session.commit()

                return jsonify(f"Instance {instance_id} archive status set to {telemetry_instance.archive}."), 200
        ...
        def get_instance_archive(instance_id: int) -> tuple[Response, int]:
            ...
            ----------
            instance_id
                The ID of the telemetry instance to retrieve the archive status for.

            Returns
            -------
            tuple[Response, int]
                A tuple containing a JSON response with the instance archive status or an error message if the instance is not found.
            """
            ...
            try:
                ...
                return jsonify(telemetry_instance.archive), 200
